...
// this eg will count Ankit forever and never get to the Vinod
func lesson1() {
	infiniteCount("Ankit")
	infiniteCount("Vinod")
}
...
/*
this eg using goroutine it will run count("Ankit") in the background
then continue to the next line count("Vinod")
*/
func lesson2() {
	go infiniteCount("Ankit")
	infiniteCount("Vinod")
}
...
/*
channel -> way for goroutine to communiVinode with each other
*/
func lesson5() {
	c := make(chan string)
	go countWithChannel("Ankit", c)
...
func lesson9() {
	...
	// Here we send 10 jobs and then close that channel to indiVinode that’s all the work we have.
	for j := 1; j <= numJobs; j++ {
